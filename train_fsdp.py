...
def setup_distributed(config):
    ...
    if config.fsdp:
        ...
        print(f"FSDP分布式训练已初始化 - Rank: {config.local_rank}, World Size: {config.world_size}")
    
    return config.local_rank == 0 or not config.fsdp  # 是否为主进程
...
def get_fsdp_wrap_policy(config):
    """获取FSDP自动包装策略"""
    from model.block import DecoderBlock
    
    # 使用transformer_auto_wrap_policy，自动包装DecoderBlock
    wrap_policy = partial(
        transformer_auto_wrap_policy,
        transformer_layer_cls={DecoderBlock},  # 指定要包装的层类型
    )
    return wrap_policy

def get_mixed_precision_policy(config):
    """获取混合精度策略"""
...
def main(config):
    ...
    # 设置随机种子（考虑分布式训练）
    seed = config.seed + (config.local_rank if config.fsdp else 0)
    ...
    if config.fsdp:
        # 获取FSDP配置
        wrap_policy = get_fsdp_wrap_policy(config)
        mixed_precision_policy = get_mixed_precision_policy(config)
        
        # 根据配置选择分片策略
        sharding_strategy_map = {
            'FULL_SHARD': ShardingStrategy.FULL_SHARD,  # 完全分片（推荐）
            'SHARD_GRAD_OP': ShardingStrategy.SHARD_GRAD_OP,  # 只分片梯度和优化器状态
            'NO_SHARD': ShardingStrategy.NO_SHARD,  # 不分片（类似DDP）
            'HYBRID_SHARD': ShardingStrategy.HYBRID_SHARD,  # 混合分片
        }
        sharding_strategy = sharding_strategy_map.get(config.sharding_strategy, ShardingStrategy.FULL_SHARD)
        
        if is_main_process:
            print(f"使用FSDP分片策略: {config.sharding_strategy}")
        ...
        model = FSDP(
            ...
            backward_prefetch=BackwardPrefetch.BACKWARD_PRE,  # 预取后向传播
            sharding_strategy=sharding_strategy,
            device_id=config.local_rank,
            limit_all_gathers=True,  # 限制all_gather操作，节省内存
        )
        
        # 获取原始模型用于优化器配置（FSDP下需要特殊处理）
        raw_model = model
    else:
        model = model.to(device)
        raw_model = model
    
    # 配置优化器（FSDP模式下直接使用包装后的模型）
    optimizer = raw_model.configure_optimizers(config.weight_decay, config.lr, (config.beta1, config.beta2), config.device)
    
    # FSDP模式下不使用compile，因为可能存在兼容性问题
    if config.compile and not config.fsdp:
        if is_main_process:
            print('compiling the model... this might take a while')
        model = torch.compile(model)
    
    # 加载checkpoint manager（FSDP模式下使用专用管理器）
    if config.fsdp:
        checkpoint_manager = FSDPCheckpointManager(config.checkpoint_root) if is_main_process else None
    else:
        checkpoint_manager = CheckpointManager(config.checkpoint_root) if is_main_process else None
    
    if config.resume_from is not None and config.resume_from != 'NULL':
        if config.fsdp:
            # 使用FSDP专用的checkpoint加载逻辑
    ...
    try:
        while global_epoch < config.max_epoch:
            ...
            for x, y in prog_bar:
                ...
                # 保存模型checkpoint
                if global_step % config.save_freq == 0 and checkpoint_manager:
                    if config.fsdp:
                        # 使用FSDP专用的保存逻辑
                        checkpoint_manager.save(model, config, global_step, global_epoch, optimizer, is_main_process)
                    else:
                        if is_main_process:
